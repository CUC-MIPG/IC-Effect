...
from models.wan_embedder import WanTimeTextImageEmbedding, WanRotaryPosEmbed
from models.wan_block import WanTransformerBlock
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def forward(
        ...
        # print(f'encoder_contion_states.shape: {encoder_contion_states.shape}, encoder_first_states shape: {encoder_first_states.shape}')
        ...
        if encoder_contion_states is not None:
            batch_size_cond, num_channels_cond, num_frames_cond, height_cond, width_cond = encoder_contion_states.shape
            scale_f = num_frames // num_frames_cond
            scale_h = height // height_cond
            scale_w = width // width_cond
            scale_factor = (scale_f, scale_h, scale_w)


            rotary_emb2 = self.rope(hidden_states_, scale_factor)
            ...
            if encoder_first_states is not None:
                ...
                raise "encoder_first_states must be provided"

        else:
            raise "encoder_contion_states must be provided"
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for block in self.blocks:
                hidden_states = self._gradient_checkpointing_func(
                    block, hidden_states, encoder_hidden_states, timestep_proj, rotary_emb, rotary_emb2, rotary_emb3, attn_mask
                )
        else:
            for block in self.blocks:
                hidden_states = block(hidden_states, encoder_hidden_states, timestep_proj, rotary_emb, rotary_emb2, rotary_emb3, attn_mask)
